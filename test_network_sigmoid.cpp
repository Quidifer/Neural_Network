...
int main() {
    cout << "This file trains and tests networks using the sigmoid activation function" << endl;
    char input;
    ...
    string output_file = "training_sets_sigmoid/";
    if (output_file == "training_sets_sigmoid/") {
        cout << "Waiting for output file. Type the name of an existing or new file: ";
        string input;
        cin >> input;
        output_file += input;
    }
    ...
    bool train = true;
    if (serializedFile) { //if the inputed file already exists
        input = 'z';
    ...
    if (serializedFile) { //if the output file is serialized
        ...
        cout << endl;
        Network::setup(num_layers, hidden_layer_size);
    }
